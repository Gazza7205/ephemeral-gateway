     ...
     stages {
        ...
        stage('Build Image with Docker') {
            steps {
                ...
                sh """docker build -t ${env.NEW_IMAGE_REGISTRY_HOSTNAME}/${env.CURRENT_IMAGE_NAME}:${env.NEW_IMAGE_TAG}  --build-arg build_number="${BUILD_NUMBER}" ."""
            }
        }
	   stage('Login Docker, Tag and push docker image to Docker Registry') {
            steps {
		        sh """docker login ${env.NEW_IMAGE_REGISTRY_HOSTNAME} -u ${params.NEW_IMAGE_REGISTRY_USER} --password ${params.NEW_IMAGE_REGISTRY_PASSWORD}
                     docker tag ${env.NEW_IMAGE_REGISTRY_HOSTNAME}/${env.CURRENT_IMAGE_NAME}:${env.NEW_IMAGE_TAG}
